...
class TranscriptionService:
    ...
    def _calculate_average_confidence(self, transcription_response):
        ...
        try:
            ...
            elif hasattr(transcription_response, 'json') and 'words' in transcription_response.json():
                words_data = transcription_response.json()['words']
            elif isinstance(transcription_response, dict) and 'words' in transcription_response:
                words_data = transcription_response['words']
    ...
    def _create_segments_from_words(self, transcription, transcription_response):
        ...
        try:
            ...
            for word in words_data:
                ...
                if (word_end - segment_start >= segment_duration) or (word == words_data[-1]):
                    ...
                    if segment_text:
                        TranscriptionSegment.objects.create(
                            ...
                            end_time=word_end,
                            text=segment_text,
                            confidence_score=self._calculate_segment_confidence(current_segment_words, words_data)
                        )
    ...
    def _create_word_timestamps(self, transcription, transcription_response):
        ...
        try:
            ...
            if not words_data:
                logger.warning("No word-level timestamps available in OpenAI response, creating mock words")
                self._create_mock_word_timestamps(transcription)
                return
            
            logger.info(f"Creating {len(words_data)} real word timestamps from OpenAI API")
            ...
            logger.warning(f"Error creating word timestamps: {str(e)}, falling back to mock")
    ...
    def _create_mock_word_timestamps(self, transcription):
        ...
        for index, word in enumerate(words):
            ...
            word_length_factor = max(0.3, min(2.0, len(word) / 5.0))  # Longer words take more time
            ...
            current_time = end_time + random.uniform(0.05, 0.15)  # Small gaps between words
