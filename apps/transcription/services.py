...
class TranscriptionService:
    ...
    def __init__(self, max_segment_duration: int = 600):
        """
        Initialize the service.

            max_segment_duration: max duration in seconds (default: 600)
        """
        ...
        try:
            ...
            if self.use_mock:
                ...
                for segment_idx, (audio_segment, segment_start_time) in enumerate(segments):
                    ...
                    # Export segment to temporary file for OpenAI API
    ...
    def _segment_audio(self, audio_file):
        """Segment audio file into chunks of max_segment_duration."""
        try:
            with audio_file.file.open('rb') as file:
                audio = AudioSegment.from_file(file)
                ...
                if duration_seconds <= self.max_segment_duration:
                    # File is short enough, return as single segment
                    return [(audio, 0.0)]
                
                segments = []
                segment_duration_ms = self.max_segment_duration * 1000  # Convert to milliseconds
                start_ms = 0
                ...
                while start_ms < duration_ms:
                    ...
                    logger.info(f"Created segment {len(segments)}: {start_time_seconds:.2f}s - {end_ms/1000.0:.2f}s")
